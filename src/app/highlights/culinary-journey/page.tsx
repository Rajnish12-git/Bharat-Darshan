                      ...
                      <div className="flex items-center gap-4">
                        {dishImage && (
                          <div className="relative h-16 w-16 rounded-md overflow-hidden flex-shrink-0">
                            <Image src={dishImage.imageUrl} alt={dish.dishName} fill className="object-cover" data-ai-hint={dishImage.imageHint}/>
                          </div>
                        )}
                        <div className="text-left">
                          {dish.dishName}
                          <p className="text-sm font-normal text-muted-foreground">{dish.region}</p>
                        </div>
                      </div>
                    </AccordionTrigger>
                    <AccordionContent className="p-4 bg-secondary/30 rounded-md">
                      <div className="space-y-4">
                        <div className="flex flex-wrap gap-2 items-center">
                           <Badge variant="secondary" className="flex items-center gap-1">{getTypeIcon(dish.type)} {dish.type}</Badge>
                           <Badge variant="secondary">Style: {dish.cookingStyle}</Badge>
                           {dish.occasions && <Badge variant="secondary">Occasions: {dish.occasions}</Badge>}
                        </div>
                        <Separator />
                        <div>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{dish.culturalSignificance}</p>
                        </div>
                        <div>
