          ...
          <div className="max-w-4xl mx-auto">
            ...
            <Accordion type="single" collapsible className="w-full">
              {festivals.map((festival) => {
                const festivalImage = PlaceHolderImages.find(
                      ...
                      <div className="flex items-center gap-4">
                        {festivalImage && (
                          <div className="relative h-16 w-16 rounded-md overflow-hidden flex-shrink-0">
                            <Image
                              src={festivalImage.imageUrl}
                              alt={festival.festivalName}
                              fill
                              className="object-cover"
                              data-ai-hint={festivalImage.imageHint}
                            />
                          </div>
                        )}
                    ...
                    <AccordionContent className="p-4 bg-secondary/30 rounded-md">
                      <div className="space-y-4">
                        <div className="flex flex-wrap gap-2 items-center text-sm">
                          ...
                          >
                            <MapPin className="h-3.5 w-3.5" /> {festival.region}
                          </Badge>
                          <Badge
                            variant="secondary"
                            className="flex items-center gap-1.5"
                          ...
                          <div className="flex flex-wrap gap-2 items-center">
                            {festival.keyElements.map((element, index) => (
                              <Badge
                                ...
                                className="flex items-center gap-1.5"
                              >
                                {element.toLowerCase().includes("light") ||
                                element.toLowerCase().includes("lamp") ||
                                element.toLowerCase().includes("fire") ? (
