...
import { Music, Landmark, Users } from "lucide-react";
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <Accordion type="single" collapsible className="w-full">
              {musicalHeritage.map((item) => {
                const itemImageId = item.instrumentName
                  ? `music-${item.instrumentName.toLowerCase().replace(/ /g, "-")}`
                  : `music-${item.musicForm.toLowerCase().replace(/ /g, "-")}`;
                const itemImage = PlaceHolderImages.find(
                  (img) => img.id === itemImageId,
                );
                const itemName =
                  item.musicForm || item.instrumentName || "Unknown";
                    ...
                    <AccordionContent className="p-4 bg-secondary/30 rounded-md">
                      <div className="space-y-4">
                        <div className="flex flex-wrap gap-2 items-center text-sm">
                          ...
                          >
                            <Music className="h-3.5 w-3.5" /> {item.category}
                          </Badge>
                          <Badge
                            variant="secondary"
                            className="flex items-center gap-1.5"
                          >
                            <Landmark className="h-3.5 w-3.5" /> {item.region}
                          </Badge>
                          {item.occasions && (
                            <Badge
                              variant="secondary"
