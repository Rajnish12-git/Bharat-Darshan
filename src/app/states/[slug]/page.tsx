        ...
        <div className="bg-background">
          <Tabs defaultValue="monuments" className="w-full">
            ...
            <div className="container max-w-7xl mx-auto py-12 px-4 md:px-8">
              <p className="text-center text-muted-foreground text-lg mb-12">{introText}</p>
              
              <TabsContent value="monuments">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                  {state.monuments.map((item) => <InfoCard item={item} key={item.name} category="monument"/>)}
                </div>
              </TabsContent>
              <TabsContent value="cuisine">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                  {state.cuisine.map((item) => <InfoCard item={item} key={item.name} category="cuisine"/>)}
                </div>
              </TabsContent>
              <TabsContent value="festivals">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                  {state.festivals.map((item) => <InfoCard item={item} key={item.name} category="festival"/>)}
                </div>
              </TabsContent>
              <TabsContent value="art">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                  {state.artForms.map((item) => <InfoCard item={item} key={item.name} category="art"/>)}
                </div>
              </TabsContent>
            </div>
          </Tabs>
