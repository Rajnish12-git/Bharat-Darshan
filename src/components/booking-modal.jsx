...
const bookingSchema = z.object({
  ...
  visitType: z.enum(["Self Visit", "Guided Tour"], {
    required_error: "Please select a visit type.",
  }),
  notes: z.string().optional(),
});

export default function BookingModal({ children, monumentName }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "We've received your request. You will be notified upon confirmation.",
      });
      form.reset();
      setOpen(false);
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            You are requesting a visit for:{" "}
            <span className="font-semibold text-primary">{monumentName}</span>.
            <br />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                type="number"
                min="1"
                {...form.register("visitors")}
              />
              {form.formState.errors.visitors && (
                <p className="text-xs text-destructive">
                  {form.formState.errors.visitors.message}
                </p>
              )}
            </div>
          </div>
            ...
            <Controller
              ...
              render={({ field }) => (
                ...
                >
                  ...
                  <Label className="flex items-center gap-2 font-normal border p-3 rounded-md has-[:checked]:bg-primary/10 has-[:checked]:border-primary transition-colors cursor-pointer">
                    <RadioGroupItem value="Guided Tour" id="guided" /> Guided
                    Tour
                  </Label>
                </RadioGroup>
              )}
